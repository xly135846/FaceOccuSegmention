...
def rec_zd_random(img, label, random_degree):
    ...
    if rnd < random_degree:
        ...
        #     if len(np.where(background_up==i)[0])!=0:
        #         print(i,len(np.where(background_up==i)[0]))
...
def get_annotation_dict(input_folder_path):
    ...
    for i in range(len(input_folder_path[:])):
        if ("quanzhedang" in input_folder_path[i]):
            ...
            label_dict[input_folder_path[i]] = input_folder_path[i].replace("_0401_crop/","_0401_crop_mask/"),\
                input_folder_path[i].replace("_0401_crop/","_0401_crop_dense/")
