...
if __name__ == "__main__":
    ...
    model = unet_vgg_model(INPUT_SZIE, 0.35)
    # model = MobileNetv2((input_size, input_size, 3), 2, t, alpha)
    # model = tfmot.quantization.keras.quantize_model(model)
    # model.load_weights(
    #     "/mnt/fu04/xueluoyang/code/segmentation_0328/checkpoints/0610_mouth_model_smaller_dice_cls_log/cp_6_0.5_64_0266.hdf5")
    model.load_weights(
        # "./checkpoints/0907_face_model_deeplap_boundary_dice_loss_01_true_log/cp_6_0.35_128_0025.hdf5",
        "./checkpoints/0915_face_model_tversky_cls_035_new_log/cp_6_0.35_128_0062.hdf5",
        )
    # print(model.summary())
    ...
    #         "/mnt/fu07/xueluoyang/data/segmentation/0610_label_zd_tfrecord/0610_label_zd_crop_val.txt")
    ...
    #             mouth_tongue = landmark[[
    #                 52, 51, 50, 49, 48, 47, 46, 61, 62, 63, 8, 7, 6]]
    ...
    #                 out_cv_3, (y_max-y_min, x_max-x_min), interpolation=cv2.INTER_AREA)
    #             # cv2.imwrite("./save_img_2/"+str(name_list[name])+"/"+files[i].split(
    ...
    # mouth
    files = get_list_from_filenames(
        "/mnt/fu07/xueluoyang/data/segmentation/0729_face_128_multi/tfrecord/fu_fprs21c_fig_val.txt")
    for i in tqdm(range(len(files[:1000]))):
        ...
        # out_canny = cv2.Canny(img, 150, 100)
        # cv2.imwrite("./save_img_5/"+str(i)+"_out_canny.png", out_canny)

        # img_YCrCb = cv2.cvtColor(img, cv2.COLOR_RGB2YCrCb)
        # print(img_YCrCb[:,:,1:3].shape)
        # cv2.imwrite("./save_img_5/"+str(i)+"_img_YCrCb_0.png", img_YCrCb[:,:,0])
        # cv2.imwrite("./save_img_5/"+str(i)+"_img_YCrCb_1.png", img_YCrCb[:,:,1])
        # cv2.imwrite("./save_img_5/"+str(i)+"_img_YCrCb_2.png", img_YCrCb[:,:,2])
        # cv2.imwrite("./save_img_5/"+str(i)+"_img_YCrCb_3.png", img_YCrCb)

        # blur_img = cv2.GaussianBlur(img, (0, 0), 5)
        # usm = cv2.addWeighted(img, 1.5, blur_img, -0.5, 0)
        # cv2.imwrite("./save_img_5/"+str(i)+"_usm.png", usm)

        cv2.imwrite("./save_img_5/"+str(i)+"_img.png", img)
        shutil.copyfile(files[i].replace("_img.png", "_mask.png"), 
                        "./save_img_5/"+str(i)+"_mask.png")
        img = np.expand_dims(img, axis=0)
        img = (img-127.5)/127.5
        out = model.predict(img)
        out_0 = out[0, :, :, 0]*255
        out_1 = out[0, :, :, 1]*255
        cv2.imwrite("./save_img_5/"+str(i)+"_pre_0.png", out_0)
        cv2.imwrite("./save_img_5/"+str(i)+"_pre_1.png", out_1)
        
        # out_canny = cv2.Canny(np.uint8(out), 50, 150)
        # cv2.imwrite("./save_img_5/"+str(i)+"_out_canny.png", out_canny)
