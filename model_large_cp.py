...
def unet_vgg_model(input_size, alpha):
    ...
    x = _up_conv(encoder.get_layer(
        "block_6_expand_relu").output, x, aaa_list[0], True)
    print("222 ", x.shape)
    x = _up_conv(encoder.get_layer(
        "block_3_expand_relu").output, x, aaa_list[1], True)
    print("333 ", x.shape)
    x = _up_conv(encoder.get_layer(
        "block_1_expand_relu").output, x, aaa_list[2], True)
    print("444 ", x.shape)
