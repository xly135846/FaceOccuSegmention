...
def get_1_4(x, indices_List_x, indices_List_y):
    x = tf.gather(x, indices=indices_List_x, axis=1)
    x = tf.gather(x, indices=indices_List_y, axis=2)
    return x

def unet_vgg_model(input_size, alpha):
    inputs = Input((input_size, input_size, 3), name="input_image")
    inputs_value = Input((2), name="inputs_value")

    print("inputs_value.shape",inputs_value[:, 0:1].shape)
    print("inputs_value.shape",inputs_value[:, 1:2].shape)
    ...
    x = encoder_output
    print("111 ", x.shape)
    x = _up_conv(encoder.get_layer("block_6_expand_relu").output, x, aaa_list[0], True)
    print("222 ", x.shape)
    x = _up_conv(encoder.get_layer("block_3_expand_relu").output, x, aaa_list[1], True)
    print("333 ", x.shape)
    x = _up_conv(encoder.get_layer("block_1_expand_relu").output, x, aaa_list[2], True)
    print("444 ", x.shape)
    x = _up_conv(encoder.get_layer("Conv1").input, x, aaa_list[3], True)
    print("555 ", x.shape)
    ...
    # x_1_4 = get_1_4(encoder_output, 
    #                 [inputs_value[:, 0:1]-i-2 for i in range(4)], 
    #                 [inputs_value[:, 1:2]-i-2 for i in range(4)])
    
    print("6666 :", [inputs_value[:, 0].tolist()-i-2 for i in range(4)])
    aaa = tf.gather(encoder_output, indices=[inputs_value[:, 0]-i-2 for i in range(4)], axis=1)
    print("aaa", aaa)
    
    # print("x_1_4 000 :", x_1_4.shape)
    # x_1_4 = _up_conv(get_1_4(encoder.get_layer("block_6_expand_relu").output, 
    #                 [i for i in range(8)],
    #                 [i for i in range(8)],
    #                 ), 
    #                  x_1_4, aaa_list[0], True)
    # print("x_1_4 111 :", x_1_4.shape)
    # x_1_4 = _up_conv(get_1_4(encoder.get_layer("block_3_expand_relu").output, 
    #                 [i for i in range(16)],
    #                 [i for i in range(16)],
    #                 ), 
    #                  x_1_4, aaa_list[1], True)
    # print("x_1_4 222 :", x_1_4.shape)
    # x_1_4 = _up_conv(get_1_4(encoder.get_layer("block_1_expand_relu").output, 
    #                 [i for i in range(32)],
    #                 [i for i in range(32)],
    #                 ), 
    #                  x_1_4, aaa_list[2], True)
    # print("x_1_4 333 :", x_1_4.shape)
    # x_1_4 = _up_conv(get_1_4(encoder.get_layer("Conv1").input, 
    #                 [i for i in range(64)],
    #                 [i for i in range(64)],
    #                 ), 
    #                  x_1_4, aaa_list[3], True)
    # print("x_1_4 444 :", x_1_4.shape)

    # x_1_4 = Conv2D(2, (1, 1), padding="same")(x_1_4)
    # x_1_4 = Activation("softmax")(x_1_4)
    # print("x_1_4 out shape ", x_1_4.shape)
...
if __name__ == '__main__':
    ...
    # params = tf.constant([[0, 1.0, 2.0, 3.0],
    #                     [10.0, 11.0, 12.0, 13.0],
    #                     [20.0, 21.0, 22.0, 23.0],
    #                     [30.0, 31.0, 32.0, 33.0]])
    # print("params :", params)
    # aaa = tf.gather(params, indices=[0,1], ).numpy()
    # print("aaa :", aaa)
    # print("aaa :", aaa.shape)

    # bbb = tf.gather(params, indices=[0,1], axis=1).numpy()
    # print("bbb :", bbb)
    # print("bbb :", bbb.shape)

    # ccc = tf.gather(aaa, indices=[0,1], axis=1).numpy()
    # print("ccc :", ccc)
    # print("ccc :", ccc.shape)
